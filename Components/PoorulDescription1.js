...
export default function PoorulDescription1({navigation,route}) {
    const{set}=route.params;
    const{lang}=route.params
    const{height,width}=Dimensions.get('screen')
    const[favoriteimg,set_favoriteimg]=useState(false) 

    const val= (lang==='Tamil'? poorularray:lang==='English'?poorulenglisharray:poorularray).find(e=>e.id===set.id)
    function handlepageingincrese()
    {
        if(set.id<108){
       let increse= (lang==='Tamil'? poorularray:lang==='English'?poorulenglisharray:poorularray).find(e1=>e1.id===set.id+1)
        navigation.navigate('PoorulDescription1',{set:increse,lang:lang})
    } else{
        navigation.navigate('InnbamDescription1',{set:(lang==='Tamil'?Innbamarray[0]:lang==='English'?InnbamEnglisharray[0]:Innbamarray),lang:lang})
    }
    }
    function handlepageingdecrease(){
        let decrease=(lang==='Tamil'? poorularray:lang==='English'?poorulenglisharray:poorularray).find(e1=>e1.id===set.id-1)
        if(decrease!=undefined){
        navigation.navigate('PoorulDescription1',{set:decrease,lang:lang})
        }else{
            navigation.navigate('AramDescription1',{set:(lang==='Tamil'?array[37]:lang==='English'?englisharray[37]:array),lang:lang})
        }
            ...
            <ScrollView>
                {
                   val.desc.map((e,index)=>
                    <TouchableOpacity onPress={()=>navigation.navigate('PoorulDescription2',{variable:e.count,lang})} key={index} style={{width:width*.97}} className='border-b-2 border-b-gray-300 px-2 py-2 '>
